...
void setup(void) {
  Serial.begin(9600);
  Serial.print("Hello! ST7628 rotation test");

  // Use this initializer if you're using a 1.8" TFT
  tft.init();   // initialize a ST7628S chip, black tab
...
void rotateFillcircle(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.fillCircle(10, 30, 10, ST7628_YELLOW);
...
void rotateDrawcircle(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawCircle(10, 30, 10, ST7628_YELLOW);
...
void rotateFillrect(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.fillRect(10, 20, 10, 20, ST7628_GREEN);
...
void rotateDrawrect(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawRect(10, 20, 10, 20, ST7628_GREEN);
...
void rotateFastline(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawFastHLine(0, 20, tft.width(), ST7628_RED);
    tft.drawFastVLine(20, 0, tft.height(), ST7628_BLUE);
...
void rotateLine(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawLine(tft.width()/2, tft.height()/2, 0, 0, ST7628_RED);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotatePixel(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawPixel(10,20, ST7628_WHITE);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateTriangle(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawTriangle(20, 10, 10, 30, 30, 30, ST7628_GREEN);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateFillTriangle(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.fillTriangle(20, 10, 10, 30, 30, 30, ST7628_RED);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateRoundRect(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawRoundRect(20, 10, 25, 15, 5, ST7628_BLUE);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateFillRoundRect(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.fillRoundRect(20, 10, 25, 15, 5, ST7628_CYAN);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
...
void rotateChar(void) {
  for (uint8_t i=0; i<4; i++) {
    ...
    tft.drawChar(25, 15, 'A', ST7628_WHITE, ST7628_WHITE, 1);
    while (!Serial.available());
    Serial.read();  Serial.read();  Serial.read();
